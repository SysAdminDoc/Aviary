...
## How do automatic Show more and Back-position restore work?

Both are on **Page cleanup** and start off. **Expand long posts automatically** waits for scrolling
to stop, then clicks only an exact Show more control owned by the outer post. It ignores quoted
posts, cards, action bars, and similar labels such as Show more replies.

**Restore position after Back** saves the current route, scroll offset, and a few visible post ids
inside the current tab. When you use the browser Back button, Aviary first returns near the saved
offset and then lines up a matching post after X has rebuilt the timeline. Move the wheel, touch the
page, press a pointer, or use a scrolling key and the restore stops immediately. Snapshots expire
after 30 minutes and disappear when the tab closes.

## Why is there no light theme?

The shipped palettes are Dim, Lights out, Graphite, Plum, Midnight, and Noir. Noir is applied by
default. Choose **Off (X's own theme)** whenever you want the site left untouched. The Control
Center keeps its dark surface even if the host page or operating system is light so contrast stays
predictable.
...
Enable **Show Copy media links** for a separate **Copy links** action. It copies the same best direct
URLs Download would use, one per line, but does not start a transfer. Only the outer post's own
media is included. Quoted-post and link-card assets keep their own controls and attribution.
...
a configured Bluesky or Mastodon account. **Delete X activity** removes the selected X activity as
soon as you press **Run**. The older Library cleanup queue remains a read-only review tool for local
records.
...
Every control Aviary offers, by Control Center page. 105 controls across 14 pages.
...
| Posts CSS | Scoped to rendered posts. Example: article { border-radius: 18px; } |
| Media actions CSS | Scoped to Aviary media buttons and their contents. |
| Navigation CSS | Scoped to X's navigation rail. |
...
| Suppress hover previews | Stop X opening a profile card or tooltip when the pointer rests on a name, avatar or control, and remove native tooltip bubbles. Menus you click, labels, and screen-reader names are left alone. |
...
| Expand long posts automatically | Click only the post's own Show more control after scrolling stops. Replies, sidebars, quoted posts, and other Show more links are left alone. |
| Restore position after Back | Return to the same visible post after opening a post or another X page and using the browser Back button. Any wheel, touch, pointer, or keyboard scroll cancels the restore immediately. |
...
| Hide For You tab | Selects Following, then collapses the first Home tab. It only affects Home and restores immediately when turned off. |
...
| Include protected posts in share exports | Protected posts stay out of HTML, Markdown, WARC, WACZ, and static viewer output until you enable this explicitly. JSON and CSV remain archival and keep the audience field. |
| Include unknown-audience posts in share exports | DOM-only, imported, and older records stay out of share-oriented exports until you enable this explicitly. JSON and CSV remain archival. |
...
| Control | What it does |
| --- | --- |
| Show download buttons | Add one Download action to each media post, plus per-asset controls. Turning this off also hides Copy media links. |
| Show Copy media links | Add a post action that copies the best direct image, GIF, video, audio, and caption URLs, one per line. It copies only that post's own media and requires the media controls above. |
...
| Local yt-dlp handoff | Offer a higher-quality adaptive save when a local authenticated helper is running. |
...
| Control | What it does |
| --- | --- |
| What to delete | Selected activity is removed in this order: bookmarks, likes, reposts, replies, posts. |
| Speed | Choose one: Careful, Balanced, Brisk. |
| Actions per batch | Pause after this many matches. Use 0 for no limit. Resume starts a fresh batch. |
| Advanced options | Change deletion speed or pause after a set number of actions. |
| Run | Sign in to X before running. |
...
## How do I save an adaptive video at the best available quality?

Direct progressive MP4 is Aviary's default and needs no extra process. If a post also exposes an
observed adaptive manifest, enable **Local yt-dlp handoff**, run `npm run yt-dlp:helper` with a
shared `AVIARY_YTDLP_TOKEN`, and enter the loopback endpoint and secret in Integrations. The post
action then offers **Send to yt-dlp** and **Copy yt-dlp command**. Only the observed manifest,
filename, and format policy leave the page. Cookies, bearer tokens, and the X post URL stay out of
the helper request.
