...
test("WACZ 1.1.1 package has checksummed STORE members and deterministic output", async () => {
  ...
  // The 1.2.0 proposal is explicitly a draft and changes the profile to `wacz` with a `home`
  // object. Keep those fields out of the published 1.1.1 target so a future refactor cannot drift
  // into the unratified shape while the version string still looks stable.
  assert.equal(datapackage.home, undefined);
  assert.equal(entries.has("indexes/index.cdx"), false);
  assert.equal(datapackage.title, "Aviary archive, 1 captured post");
  assert.match(datapackage.description, /local WACZ export generated by Aviary/);
  assert.equal(datapackage.created, generatedAt.toISOString());
  assert.equal(datapackage.modified, generatedAt.toISOString());
  assert.equal(datapackage.mainPageUrl, "https://aviary.invalid/pages/000001-1.html");
  assert.equal(datapackage.mainPageDate, "2026-08-12T12:00:00Z");
...
test("CDXJ offsets address replayable WARC records and pages use RFC3339 timestamps", async () => {
  ...
  for (const line of cdxLines) {
    ...
    assert.match(String(metadata.status), /^\d{3}$/);
    if (metadata.url.includes("pbs.twimg.com")) {
      ...
      assert.match(record, /cache-control: public, max-age=60\r\n/i);
  ...
  assert.equal(pageLines[1].capturedAt, "2026-08-12T12:00:00Z");
...
test("WACZ keeps replay timestamps tied to capture time and exposes authored time", async () => {
  const [{ buildWaczArchive }, { readStoreZip }] = await Promise.all([
    importSourceModule("src/features/export/wacz.ts"),
    importSourceModule("src/features/export/zip-reader.ts")
  ]);
  const artifact = buildWaczArchive([sampleRecord({
  ...
  const entries = new Map(readStoreZip(artifact.data).map((entry) => [entry.filename, entry.data]));
  const pages = decoder.decode(entries.get("pages/pages.jsonl")).trim().split("\n").map(JSON.parse);
  assert.equal(pages[1].ts, "2026-08-12T12:00:00Z");
  assert.equal(pages[1].capturedAt, "2026-08-12T12:00:00Z");
  assert.equal(pages[1].publishedAt, "2026-08-01T09:30:00Z");
  const warcText = decoder.decode(entries.get("archive/aviary.warc"));
  assert.match(warcText, /WARC-Date: 2026-08-12T12:00:00Z/);
  assert.match(warcText, /Posted 2026-08-01T09:30:00.000Z/);
  assert.match(warcText, /Captured 2026-08-12T12:00:00.000Z/);
});

test("captured bytes without a received status stay resources and stay out of CDXJ", async () => {
  const [{ buildWaczArchive }, { readStoreZip }] = await Promise.all([
    importSourceModule("src/features/export/wacz.ts"),
    importSourceModule("src/features/export/zip-reader.ts")
  ]);
  const artifact = buildWaczArchive([sampleRecord({
    media: [{
      kind: "photo",
      url: "https://pbs.twimg.com/media/unknown.jpg",
      bytes: new TextEncoder().encode("unknown status"),
      type: "image/jpeg"
    }]
  })], { generatedAt: new Date("2026-08-12T12:34:56Z") });
  const entries = new Map(readStoreZip(artifact.data).map((entry) => [entry.filename, entry.data]));
  const cdx = decoder.decode(entries.get("indexes/index.cdxj"));
  assert.doesNotMatch(cdx, /unknown\.jpg/);
  const warcText = decoder.decode(entries.get("archive/aviary.warc"));
  const mediaRecord = warcText.match(/WARC-Type: resource\r\nWARC-Target-URI: https:\/\/pbs\.twimg\.com\/media\/unknown\.jpg[\s\S]*?(?=WARC\/1\.1|$)/)?.[0] ?? "";
  assert.match(mediaRecord, /unknown status/);
  assert.doesNotMatch(mediaRecord, /HTTP\/1\.1 200 OK/);
});
