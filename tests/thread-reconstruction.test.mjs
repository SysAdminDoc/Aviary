...
test("reconstructThreads merges overlapping contexts, orders parents, and marks conversations", async () => {
  const { reconstructThreads, reconstructExportOrder } = await importSourceModule("src/features/export/thread-reconstruction.ts");
  const records = [
...
test("reconstructThreads surfaces missing parents and collapses same-author runs", async () => {
  const { reconstructThreads } = await importSourceModule("src/features/export/thread-reconstruction.ts");
