# Design QA

Date: 2026-09-23

<!-- docs-facts:start -->

<!-- Generated by tools/docs-facts.mjs from the manifests and the panel's own metadata.
     Edit those, then run `npm run docs:facts`. -->

Aviary 1.54.0: 15 destinations, 77 committed PNG baselines, 21 selector surfaces.

<!-- docs-facts:end -->

## v1.54.0 Noir layout and interaction audit

- Checked signed-in Home, Profile, post detail, and a long reply stream at 1920 by 1080. Profile
  was checked again at a 1280 by 900 viewport.
- Wide fills ordinary desktop room and centers a 1,180px lane when more space is available. The
  live profile measured 990px at 1280 and 1,180px at 1920, with no horizontal overflow.
- Direct post media is capped at 1,040px. Post actions form a centered 760px group with 36px targets,
  so the controls remain easy to scan without stretching from edge to edge.
- Profile identity content is centered at up to 960px. Follow and Edit profile use Noir's control
  shapes, while the verification card now uses a quiet raised surface.
- X's current relative-positioned reply connector is detected and removed. Action glyphs now take
  the same muted, active, or destructive color as their button.
- Collapsed Grok and Chat drawers are hidden in Wide mode instead of covering the final controls.
  Repeated theme passes leave settled markers and root attributes untouched.
- The accepted synthetic screenshots are stored under `tests/visual/baselines`. Live-account audit
  screenshots remain in the task evidence folder and contain no credentials.

## v1.53.0 Noir live X audit

- Verified Home, Explore, History, Profile, and post-detail routes with the signed-in test account.
- The navigation no longer shifts when hovered. Active route styling now covers History and Grok,
  including nested History pages that don't expose X's older tab test ids.
- Repeated theme passes leave stable navigation, conversation, stream, and media markers untouched.
  Media geometry is read once for a settled frame instead of again on every timeline mutation.
- Wide keeps a 990px primary lane at the 1280 by 720 live viewport with no horizontal overflow.
  Collapsed Grok and Chat drawers use 56px instead of covering the post action row.
- Noir uses quieter steel-blue accents, softer dividers, an opaque reading surface, and one row
  hover layer. The Post button keeps the authored fill instead of inheriting X's light inline fill.

## v1.52.2 live X layout audit

- Verified Home, Search, Profile, and post-detail routes with the signed-in test account.
- Checked desktop widths of 2851, 1920, and 1440 pixels, then compact widths of 768 and 390 pixels.
- The real timeline lane now expands with Wide instead of staying at X's internal 600px cap.
- At 2851px, live post text and action rows measured 2419px with no character-count cap. The page
  stayed free of horizontal overflow.
- Primary media grows according to its shape until it reaches 72% of the viewport height. Quoted
  media stays within 720px.
- Profile identity content stays on a 960px lane. The avatar is 160px on desktop and 112px on
  compact screens.
- Video, portrait image, multi-image gallery, quoted-post, reply, and settings-panel states had no
  horizontal page overflow. The live browser console showed no Aviary errors.

## Control Center clarity

- The first view is **Quick setup**, with four task cards for quieting X, filtering posts,
  downloading media and deleting account activity.
- Eight common destinations remain visible. Seven specialist destinations sit under one
  **More tools** disclosure, but remain searchable and directly addressable.
- The footer explains that switches and fields wait for **Save**, while action buttons run
  immediately. The distinction stays visible at desktop widths without competing with the page.
- Saved posts now begins with universal search and bookmarks. Storage reports moved to the bottom.
  Custom CSS moved to **Advanced styling** on **Look & feel**.
- Reviewed dark and light fixtures at 1440 by 900, plus the 768px narrow layout. Task cards collapse
  to one column on the narrow layout, and the panel has no horizontal overflow.

## v1.51.2 Delete X activity flow

- The account cleanup page presents one filled **Run** action before the collapsed **Advanced
  options** disclosure. Pressing it starts deletion immediately. There is no preview or arming
  field.
- Active passes replace the primary action with Pause or Resume and Stop. A failed start
  rebuilds the idle state with an enabled Run button.
- Active status copy now distinguishes actions, older-item loading, end-of-list checks, timed rests,
  X's 500-action Like window, and automatic continuation. Balanced is selected for a new run unless
  an earlier run saved a different speed.
- A non-responsive action keeps the current item selected for retry. Recovery status explains when
  the current X page reloads and how many automatic reload attempts remain.
- Category completion status explains the fresh top-of-list verification reload. A category does
  not advance until the reloaded route contains no matching actions.
- Reviewed the account page in dark and light fixtures at 1440 by 900 and 1920 by 1080. The primary
  action stays in the first viewport, and the narrow layout stacks its input and button cleanly.

## v1.51 reading and media controls

- Reviewed the new Page cleanup and Downloads rows in dark and light fixtures at 1440 by 900 and
  1920 by 1080. Each label states its trigger, boundary, and dependency without a separate help
  panel.
- Added dark and light injected baselines with **Copy links** enabled on a photo-only post. The
  outlined action remains secondary to Download, stays readable on both X themes, and fits the
  native action row without clipping.
- Kept the pending-video baseline. Copy links does not appear while a mixed post's direct video is
  unresolved, so it cannot copy an incomplete asset list.

## Reference comparison

- Compared the selected Quiet Console references in `docs/mockups/2026-08-20/` with the matching
  dark 1440 by 900 implementation captures in `tests/visual/baselines/settings/`.
- Appearance now uses the same flat reading surface, compact header, single teal accent, quiet
  dividers, and larger control labels as the reference.
- Media matches the reference hierarchy more closely. Its three batch settings share one row, and
  Download all visible media is a filled primary action inside the first viewport.
- The extension permissions page keeps the real grant and revoke workflow from the source product.
  Its visual treatment follows the reference direction with a compact masthead, one-column content,
  plain status text, and one clear primary button.

## Intentional differences

- The Control Center remains a modal over X instead of becoming a full-window application. This
  preserves the existing entry and return flow.
- The mockups occasionally invented controls or removed required product content. The implementation
  keeps the current feature model and uses the mockups only for hierarchy, density, typography, and
  control grouping.
- Helper text is clamped instead of deleted. Settings that need context still expose it without
  overpowering the control.

## Captured media workflow

- Generated a focused Library reference at
  `docs/mockups/2026-08-21/control-center-library-captured-media.png`, using the established Library
  page as the edit target.
- The implementation keeps search dominant, places one compact Download media action beside it,
  and shows a quiet media count. It does not add another card, pill, or explanatory panel.
- Media keeps the new sidecar selector in the file-naming group. Its label, short description, and
  standard select treatment match the surrounding controls.
- Reviewed the dark 1440 by 900 and 1920 by 1080 Library captures against the generated reference.
  The modal entry flow and existing bookmark tools remain intentional differences.

## Portable filtering rules

- Compared the Filtering implementation with `docs/mockups/2026-08-20/control-center-filtering.png`.
- Export, paste, preview, add, and replace controls share one compact row instead of creating five
  bordered settings cards. The preview sits directly under the controls and uses plain status text.
- Reviewed the dark 1440 by 900 and 1920 by 1080 captures. The rule editor remains the dominant
  control, button labels stay visible, and the row fits without horizontal overflow.
- Exercised invalid and valid paste states in Chromium. Parse errors name their source line, both
  apply buttons stay disabled on invalid input, and the valid preview exposes both outcomes.

## Preservation export

- Compared the Export implementation with `docs/mockups/2026-08-20/control-center-export.png`.
- WARC, WACZ, and replayweb.page now share one preservation row. WACZ is the only filled action,
  while the raw format and viewer link stay secondary.
- The storage estimate sits in the helper line before the action runs. No new card, status pill, or
  confirmation layer was added.
- Added a focused dark 1440 by 900 baseline at
  `tests/visual/baselines/settings/control-center-export-preservation-dark-1440x900.png` so this
  below-the-fold action remains part of visual review.

## Verification

- Reviewed all 15 Control Center destinations on dark and light X fixtures at 1440 by 900 and 1920
  by 1080.
- Reviewed saved, error, focus, reduced-motion, and disabled-permission states.
- Confirmed the narrow layout returns Media to one column with no horizontal overflow.
- Confirmed the first-run notice clears while the Control Center is open.
- Confirmed the feed Download action stays enabled while direct video metadata is pending.

## X theme parity

- Compared the selected Quiet Stream direction at
  `docs/mockups/2026-08-20-x-theme/direction-b-quiet-stream.png` with the Home and conversation
  implementation captures at the same 1440 by 900 state. The combined review image is
  `docs/mockups/2026-08-20-x-theme/quiet-stream-comparison.png`.
- The 1120px Wide canvas now removes the discovery rail and centers posts. Comfortable keeps the
  rail with a 920px reading column.
- Home posts now form one continuous stream with flat surfaces, quiet separators, restrained hover
  feedback, 10px media corners, and action rows that use the full post width.
- Conversation routes identify one focal post and render replies as a compact connected stream.
  Focal text has stronger hierarchy, the reply composer is shorter, and reply text keeps a readable
  measure inside the wider column.
- Reviewed Home and conversation captures in the in-app browser at 1440 by 900. Repeated captures at
  1920 by 1080 confirmed the reading column remains centered without horizontal overflow.
- Checked all six authored themes against dark and light host fixtures at both desktop sizes. Off
  removes the surface marker and all conversation-role attributes.

final result: passed
